    ...
    @else
        <table class="table table-hover">
            ...
            <tbody>
                {{-- // @forelse ($adoptions->chunk(10) as $chunkedadoptionss) --}}

                @foreach ($petshooterapp as $item)
                    <tr class="clickable-row"
                        onclick="window.location='{{ url('/petshooterApplication/received/' . $item->petshooterAppID) }}'"
                        ...
                        <td>
                            @if ($item->verification_status == 'Pending')
                                ...
                                {{-- @elseif ($item->verification_status == 'Deactivated')
